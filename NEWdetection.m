...
%intital things for code

%when code is run multiple times it ensures that no old variables are taken
clear
...
B = rgb2gray(A);
[regions] = detectMSERFeatures(B, 'ThresholdDelta', 2, 'RegionAreaRange', [250,300], 'MaxAreaVariation', 0.05 );
%originally 150,400
...
while n < limit
    ...
    while m < (limit+1)
        ...
        if distance(1)<30 && distance(2)<30
            regions(m) = [];
            limit = limit - 1;
        end
        m = m+1;
...
for n = 1:regions.Count
    ...
    %read the crops
           ...
           numberIm(n,1) = numberDomino2(cropIm{n,1});
           numberIm(n,2) = numberDomino2(cropIm{n,2});

    %save value into mega array

            Domino(n).Value = numberIm(n,:);
